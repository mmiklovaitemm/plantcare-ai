    ...
    >
      ...
      {entry.photo_url && (
        <div className="relative w-full aspect-[4/3] overflow-hidden">
          ...
          <button
            ...
            className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 p-1.5 bg-black/50 rounded-lg text-white hover:bg-red-500/80 transition-all"
          >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            {entry.plants?.photo_url ? (
              <img src={entry.plants.photo_url} alt={entry.plants.nickname} className="w-6 h-6 rounded-lg object-cover" />
            ) : (
              <div className="w-6 h-6 rounded-lg bg-[#E8F5EC] dark:bg-[#0f2a18] flex items-center justify-center">
                <Leaf className="w-3 h-3 text-[#86EFAC]" />
              </div>
            )}
            <span className="text-sm font-semibold text-[#1A1A1A] dark:text-white">{entry.plants?.nickname}</span>
          </div>
          <span className="text-xs text-[#6B7280] dark:text-[#9ca3af]">{timeAgo(entry.recorded_at)}</span>
        </div>
        ...
        {!entry.photo_url && (
          <button
            onClick={() => onDelete(entry.id)}
            className="mt-2 opacity-0 group-hover:opacity-100 text-xs text-red-400 hover:text-red-500 transition-all flex items-center gap-1"
          >
